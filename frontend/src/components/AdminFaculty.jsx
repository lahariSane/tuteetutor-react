...
const UserCard = ({ user, token, onUserRemove, onEditUser, userRole }) => {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/user/${user._id}`, {
...
const UserList = () => {
  ...
  return (
    <>
      ...
      >
        {userRole === "admin" && (<div><AddIcon sx={{ marginRight: "6px" }} />Add user</div>)}
        {userRole === "hod" && (<div><AddIcon sx={{ marginRight: "6px" }} />Add Faculty</div>)}
      </Fab>
    </>
  );
};

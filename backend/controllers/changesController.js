...
class ChangesController {
    ...
    async createChange(req, res) {
        const existing = await Changes.findOne({ date, month, year });
        if (existing) {
            return res.status(409).json({ message: "Change already exists for this date." });
        }
        try {
            ...
            if (error.code === 11000) {
                res.status(409).json({ message: "Duplicate date/month/year entry." });
            } else {
